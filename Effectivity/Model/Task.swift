...
class PersistedTask: Object {
    ...
    @Persisted var priority: PersistablePriority
    @Persisted var deadline: Date
    
    convenience init(id: String = UUID().uuidString, title: String, text: String, priority: PersistablePriority, deadline: Date) {
        self.init()
        self.id = id
        self.title = title
        self.text = text
...
enum PersistablePriority: Int, PersistableEnum {
    case urgently = 0
    case nonUrgently = 1
    case usually = 2
}
